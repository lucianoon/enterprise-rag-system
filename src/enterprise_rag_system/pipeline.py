...
class ResultReranker(Protocol):
    """Reorders first-stage results (legacy title bonus or a cross-encoder)."""
...
class RAGPipeline:
    ...
    def __init__(
        ...
        *, retrieval_mode: RetrievalMode = "hybrid",
        ...
        if rerank_pool is not None and rerank_pool < 1:
        ...
        self.reranker: ResultReranker = reranker or Reranker()
        # Candidates handed to the reranker; defaults to twice the requested K.
    ...
    def retrieve(
        ...
        pool = top_k * 2
        if rerank and self.rerank_pool is not None:
            pool = max(pool, self.rerank_pool)
        initial = self.retriever.search(question, top_k=pool, mode=mode)
